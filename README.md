...
- **Per-member effort pins** — in the composer's Options, pin any single member (or the judge) to its own effort level for that question; per-model beats per-call beats the council default. Built for mixed councils where one member prices effort very differently (measured: 25× latency spread on the same model between low and max).
...
- **Save the whole run to a file** — the composer's Options take an absolute `.md`/`.txt`/`.json` path and the *server* writes the complete result there after the run: every member's full response, judge output, sources, usage, and member-written files inlined. The result view shows the save receipt (or the write error — a bad path never costs the answer).
- **Member files for big reviews** — an auto/on/off control forwards `member_file_output`: members stream long findings to private, write-confined scratch directories (auto-on for repo/web asks, where full findings otherwise get truncated), collected after the run and listed in the result view. Where they land is a Council panel setting (**Member files location**, default the server's OS temp directory; stale runs sweep after 7 days).
- **Harness tool concurrency** — Council panel setting for how many tools one Claude-CLI member may run in parallel inside its own session (web fetches, repo reads); overrides any throttle the member would otherwise inherit from your interactive session.
