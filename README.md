...
- **Member files for big reviews** — an auto/on/off control forwards `member_file_output`: members stream long findings to private, write-confined scratch directories (auto-on for repo/web asks, where full findings otherwise get truncated), collected after the run and listed in the result view.
